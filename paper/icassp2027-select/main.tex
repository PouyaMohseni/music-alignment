...
\title{SCORE FOLLOWING IN SHEET MUSIC IMAGES\\AS SEQUENTIAL CANDIDATE SELECTION}
...
proposes a correct position at every scored onset and an oracle held to the
same realtime commitments recovers almost everything the deployed rule loses. We therefore recast the
...
already saw, which indicates that reverberation degrades the detector's
confidence ordering rather than the underlying visual evidence.
...
on real recordings by a margin that holds across three independently trained
perception models.
...
final step. Where the detector reports the single most confident box, \sys{}
treats the boxes as a candidate set and orders them by how well each continues
the performance so far, carrying its own state from one instant to the next.
...
The four constants $(\mu,\sigma,J,\beta)$ are fitted leave-one-piece-out, so for
...
\begin{table*}[t]
\centering
\caption{Real piano recordings, 16 pieces and 4,149 onsets. Ratio of onsets
tracked within each error threshold. Layout marks methods needing system and
...
rec@1 is the fraction first tracked correctly within 1\,s, post the onsets
tracked within 1\,s over the next 5\,s.}
\label{tab:jump}
\setlength{\tabcolsep}{4pt}
{\footnotesize
\begin{tabular}{lcccc}
...
performs considerably worse than the rule it replaces, 0.08 against 0.27 of onsets tracked over the
...
trigger for any repertoire in which repeats are taken. A transition
...
recovers almost all of what the deployed rule loses, so the information needed
to follow the performance survives the room and what fails is the rule that
reads a position out of it.
...
against 88.3\% for the best published system, at 1.25\,ms per instant on a
single CPU thread. The margin holds across three independently trained
perception models, which suggests the finding concerns the family of
audio-conditioned detectors rather than one checkpoint, and it is obtained
without retraining any of them and without layout annotation at test time.
...
The method applies where a follower discards a candidate set, which is what
makes the detection-based family its natural target. CODA~\cite{yang2026coda}
is not such a target, since its final stage regresses a continuous bar-local
coordinate rather than choosing among proposals, and the sets it does select
over, systems and bars, are supplied by layout annotation and already decoded
with learned transition penalties and beam search. Comparing that decoder with
ours over the same candidates is a worthwhile experiment and a different one. Two results temper this. Accuracy on synthesized audio
