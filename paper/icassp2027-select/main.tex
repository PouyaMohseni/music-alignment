...
to a position at every instant with no lookahead. Doing it directly on a
sheet-music image converts neither modality, avoiding both optical music
recognition and transcription, but such systems are trained on synthesized
audio for want of annotated real recordings and lose much of their accuracy on
a real instrument in a real room. Closing that gap is the field's central open
problem, and every remedy proposed for it has treated the gap as a failure of
perception. We show it is not. On real recordings a conditional YOLO detector
already proposes a correct position at every scored onset, and an oracle under
the same real-time constraints, committing at each instant and never moving
backwards, reaches 99.6\% where the deployed rule reaches 80.0\%. We therefore
...
scorer that judges each candidate against its competitors. On real piano
recordings this tracks 94.4\% of onsets within 0.5\,s against 88.3\% for the
best published system, and holds that margin across three independently trained
detectors. What the decision needs, we find, is its own temporal history rather
than a better image representation.
\end{abstract}
