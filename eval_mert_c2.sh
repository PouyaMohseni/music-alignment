#!/bin/bash
#SBATCH --job-name=eval-mert-c2
#SBATCH --account=def-ichiro
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=4:00:00
#SBATCH --output=/project/def-ichiro/pmohseni/music-alignment/results/eval_mert_c2-%j.log
#SBATCH --error=/project/def-ichiro/pmohseni/music-alignment/results/eval_mert_c2-%j.log

# Interim spot-check of MERT_C2_soft_dtw on its current best_model.pt --
# training is still in progress as of submission, so this is not a final
# result. Uses the MERT-aware native eval (extensions/hooks/
...
CKPT_DIR=$(find results/cb_ta_ext/MERT_C2_soft_dtw/params -maxdepth 1 -name "*_MERT_C2_soft_dtw" -type d 2>/dev/null | sort | tail -1)
...
echo "=== Native MERT-aware eval_model.py: MERT_C2_soft_dtw on msmd_test ==="
