...
The last arm is the shipped checkpoint: SHIP=<path> overrides it, so the table
follows whichever model the pages report instead of hard-coding one."""
...
SHIP = os.environ.get('SHIP', f'{M}/grid/vel_p8.pt')
S = SHIP.split('/')[-1][:-3]
print(f'shipped arm: {SHIP}')
...
arms[S], _ = rollout_hits(load_ckpt(SHIP)[0], pages)
cl = piece_of(pg)
rows = []
for p in sorted(set(cl)):
    ...
    rows.append((p, int(m.sum()),
                 *[100.0 * arms[k][m].mean() for k in
                   ('argmax', 'hand', 'ir_only', S)]))
rows.sort(key=lambda r: -(r[5] - r[2]))
print(f'{"piece":52s} {"n":>5s} {"argmax":>7s} {"hand":>7s} {"ir_only":>8s} {S[:10]:>10s} {"d":>7s}')
for r in rows:
    print(f'{r[0][:52]:52s} {r[1]:5d} {r[2]:7.1f} {r[3]:7.1f} {r[4]:8.1f} {r[5]:10.1f} {r[5]-r[2]:+7.1f}')
tot = [100.0 * arms[k].mean() for k in ('argmax','hand','ir_only',S)]
