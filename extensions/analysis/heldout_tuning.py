"""Choose the decoder's constants on held-out pieces, then read room once.
...
blend weight with the reported scorer, both on the 80 held-out pieces at 12 dB,
then reports the room number of the chosen setting next to the shipped one.

It also measures the reference step Delta_0 (REF_FRAMES = 5, documented as the
room set's median onset gap) on the TRAINING dumps, so the paper can cite a
training-set statistic for it instead.
"""
...
def main():
    ...
    print(f'\nheld-out prior: best fwd={best[0]} sigma={best[1]} J={best[2]} '
    ...
    for tag, (fwd, sig, jump) in settings:
        for b in BLENDS:
            hits, _ = rollout_hits(m, hv, blend=b, fwd=fwd, sigma=sig, jump=jump)
    ...
    print(f'\nheld-out blend at the chosen prior: {b_star:.1f} '
