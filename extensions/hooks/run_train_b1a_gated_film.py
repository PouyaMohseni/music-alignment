"""B1a + gated FiLM entry point: B1a's frozen-MERT audio-encoder swap
(extensions/hooks/mert_patch.py) PLUS replacing FiLM with GatedFiLM
(extensions/hooks/gated_film_patch.py, AdaLN-Zero-style zero-initialized
gate) -- tests whether ramping in the audio conditioning's influence from
zero, instead of applying it at full random-initialized strength from step
one, improves training. Visual side is still CB_TA's own from-scratch conv
