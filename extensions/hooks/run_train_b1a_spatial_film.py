"""B1a + spatial FiLM entry point: B1a's frozen-MERT audio-encoder swap
(extensions/hooks/mert_patch.py) PLUS replacing FiLM with SpatialFiLM
(extensions/hooks/spatial_film_patch.py, SPADE-inspired spatially-varying
gamma/beta) -- tests whether letting the audio conditioning vary across the
image (via a learned coarse pattern, no real attention/content-awareness)
beats stock FiLM's single global scale/shift. Visual side is still CB_TA's
