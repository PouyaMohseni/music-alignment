"""B1a + cross-attention entry point: B1a's frozen-MERT audio-encoder swap
(extensions/hooks/mert_patch.py) PLUS replacing FiLM with
SpatialCrossAttentionFiLM (extensions/hooks/cross_attention_film_patch.py) --
the genuine fusion-mechanism change requested as a follow-up to B1a (which
only ever swapped the audio *source*, leaving FiLM's context-blind broadcast
untouched). Visual side is still CB_TA's own from-scratch conv encoder,
unchanged -- only the conditioning mechanism changes.
