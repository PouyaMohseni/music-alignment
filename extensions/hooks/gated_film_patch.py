...
site uses GatedFiLM (extensions/heads/gated_film.py, AdaLN-Zero-style zero-
initialized gate) instead of stock FiLM. Everything else in ConditionalUNet
is untouched, so this composes with any audio-encoder patch (e.g.
extensions/hooks/mert_patch.py for B1a) applied before or after it.
...
def patch_gated_film():
    ...
    # which orthogonal-inits every nn.Linear/nn.Conv2d it finds -- including
    # GatedFiLM's gate, clobbering its zero-init AFTER construction (caught
    # by the integration smoke test: audio leaked through at "init" despite
    # gate being zero-initialized in GatedFiLM.__init__). Wrap
    # initialize_weights so any module tagged _gated_film_zero_init gets
    # re-zeroed instead of orthogonal-initialized.
    ...
    class ConditionalUNetBlockGatedFiLM(nn.Module):
        def __init__(self, in_channels, out_channels, spec_out=128, film=True, down_sample=True,
            ...
            if self.film:
                self.film_layer = GatedFiLM(spec_out, out_channels)
    ...
    cpjku_network.ConditionalUNetBlock = ConditionalUNetBlockGatedFiLM
    print('[gated_film_patch] Replaced FiLM with GatedFiLM (AdaLN-Zero-style zero-initialized '
          'gate, block starts as identity) in ConditionalUNetBlock', flush=True)
