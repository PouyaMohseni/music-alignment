...
site uses SpatialFiLM (extensions/heads/spatial_film.py, SPADE-inspired
spatially-varying gamma/beta) instead of stock FiLM. Everything else in
...
cross_attention_film_patch.py's docstring for why (the original __init__'s
explicit `super(ConditionalUNetBlock, self)` call breaks under monkey-
patching). This is a fully independent nn.Module, structurally identical to
the original, with only the FiLM line swapped.
"""
...
def patch_spatial_film():
    ...
    class ConditionalUNetBlockSpatialFiLM(nn.Module):
        def __init__(self, in_channels, out_channels, spec_out=128, film=True, down_sample=True,
            ...
            if self.film:
                self.film_layer = SpatialFiLM(spec_out, out_channels)
    ...
    cpjku_network.ConditionalUNetBlock = ConditionalUNetBlockSpatialFiLM
    print('[spatial_film_patch] Replaced FiLM with SpatialFiLM (SPADE-inspired spatially-'
          'varying gamma/beta from a coarse learned grid) in ConditionalUNetBlock', flush=True)
