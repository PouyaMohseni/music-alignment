...
#SBATCH --output=/project/def-ichiro/pmohseni/music-alignment/results/mert_dinov2-%j.log
#SBATCH --error=/project/def-ichiro/pmohseni/music-alignment/results/mert_dinov2-%j.log

# Frozen MERT audio + a constant-per-page DINOv2 CLS embedding, concatenated
# into a single 1536-dim per-frame input (extensions/hooks/mert_dinov2_patch.py).
# Low-risk half of the visual-pretraining experiment: the existing
# from-scratch visual encoder/decoder/skip-connections are completely
# untouched, this only adds pretrained global visual context alongside the
# audio signal. B1a alone (frozen MERT, no visual pretraining): 88.9%
# pct@0.5s, current project best.
...
OUT=/project/def-ichiro/pmohseni/music-alignment/results/cb_ta_ext/MERT_DINOv2_bottleneck
...
echo "=== MERT+DINOv2 (bottleneck): frozen MERT audio + DINOv2 global visual context ==="
cd "$REPO/audio_conditioned_unet"

python /project/def-ichiro/pmohseni/music-alignment/extensions/hooks/run_train_mert_dinov2.py \
    ...
    --audio_encoder MERTDINOv2Projector \
    --tag MERT_DINOv2_bottleneck \
    $PARAM_FLAG
