#!/bin/bash
#SBATCH --job-name=b1a-gated-film
...
#SBATCH --output=/project/def-ichiro/pmohseni/music-alignment/results/b1a_gated_film-%j.log
#SBATCH --error=/project/def-ichiro/pmohseni/music-alignment/results/b1a_gated_film-%j.log

# B1a + GatedFiLM (AdaLN-Zero-style): FiLM's gamma/beta modulation is now
# blended in via a THIRD, zero-initialized gate value, so every FiLM block
# starts as an exact identity function (audio has zero effect at init) and
# gradually learns how much to trust the audio conditioning, instead of
# applying full-strength, randomly-initialized modulation from step one.
# Visual side (CB_TA's own from-scratch conv encoder) is unchanged -- only
# the conditioning mechanism changes. Smoke-tested: gate=0 gives an EXACT
# identity (verified numerically, not just shape-checked) at both the
# isolated-module and whole-network level.
...
OUT=/project/def-ichiro/pmohseni/music-alignment/results/cb_ta_ext/B1a_gated_film
...
echo "=== B1a + GatedFiLM (AdaLN-Zero-style zero-init gate) ==="
cd "$REPO/audio_conditioned_unet"

python /project/def-ichiro/pmohseni/music-alignment/extensions/hooks/run_train_b1a_gated_film.py \
