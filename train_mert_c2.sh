...
#SBATCH --output=/project/def-ichiro/pmohseni/music-alignment/results/mert_c2-%j.log
#SBATCH --error=/project/def-ichiro/pmohseni/music-alignment/results/mert_c2-%j.log

# MERT audio-encoder swap (frozen, precomputed embeddings, same as B1a)
# COMBINED with C2's soft-DTW auxiliary training loss. C2 alone (original
# CBEncoder) scores 85.0% pct@0.5s; B1a alone (frozen MERT, no aux loss)
# has historically underperformed CBEncoder. Tests whether the soft-DTW
# training signal transfers on top of MERT features even if raw MERT
# doesn't beat CBEncoder by itself.
...
OUT=/project/def-ichiro/pmohseni/music-alignment/results/cb_ta_ext/MERT_C2_soft_dtw
...
echo "=== MERT+C2: frozen MERT audio encoder + soft-DTW aux loss ==="
cd "$REPO/audio_conditioned_unet"

python /project/def-ichiro/pmohseni/music-alignment/extensions/hooks/run_train_mert_c2.py \
