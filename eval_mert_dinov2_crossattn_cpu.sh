#!/bin/bash
#SBATCH --job-name=eval-mert-dinov2-xattn-cpu
#SBATCH --account=def-ichiro
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=8:00:00
#SBATCH --output=/project/def-ichiro/pmohseni/music-alignment/results/eval_mert_dinov2_xattn_cpu-%j.log
#SBATCH --error=/project/def-ichiro/pmohseni/music-alignment/results/eval_mert_dinov2_xattn_cpu-%j.log

# First-ever eval of MERT_dinov2_cross_attention (the "actual new method":
# MERT audio + DINOv2 visual tokens + TokenCrossAttentionFiLM as the fusion
# mechanism, replacing FiLM's context-blind broadcast entirely). CPU-only:
# both MERT embeddings and DINOv2 tile grids are precomputed on disk, only
...
echo "=== Native eval_model.py: MERT_dinov2_cross_attention on msmd_test (CPU) ==="
