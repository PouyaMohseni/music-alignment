#!/bin/bash
#SBATCH --job-name=eval-b4
#SBATCH --account=def-ichiro
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=4:00:00
#SBATCH --output=/project/def-ichiro/pmohseni/music-alignment/results/eval_b4-%j.log
#SBATCH --error=/project/def-ichiro/pmohseni/music-alignment/results/eval_b4-%j.log

# Interim spot-check of B4_temporal_consistency on its current best_model.pt
...
# native portrait per-page format B4 was actually trained on (e.g. 1181x835,
...
CKPT_DIR=$(find results/cb_ta_ext/B4_temporal_consistency/params -maxdepth 1 -name "*_B4_temporal_consistency" -type d | sort | tail -1)
...
echo "=== Native eval_model.py: B4_temporal_consistency on msmd_test ==="
