#!/bin/bash
#SBATCH --job-name=eval-msmd-rec-mert
#SBATCH --account=def-ichiro
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=4:00:00
#SBATCH --output=/project/def-ichiro/pmohseni/music-alignment/results/eval_msmd_rec_mert-%j.log
#SBATCH --error=/project/def-ichiro/pmohseni/music-alignment/results/eval_msmd_rec_mert-%j.log
...
# MERT checkpoints: always the MERT wrapper, and MERT_TEST_EMB_ROOT MUST point
# at embeddings encoded from THESE REAL wavs. mert_patch never encodes audio at
# eval time -- it loads {emb_root}/{piece}_tempo_{cond}.npy. Leaving the stale
# synthetic root here would score SYNTHETIC audio while the log claims a
# real-audio result, which is a silent, publication-grade wrong number.
export MERT_TEST_EMB_ROOT=/scratch/pmohseni/mert_emb_msmd_rec/${COND}
export MERT_EVAL_TEST_DIR="$REC_DIR"          # must equal the --test_dir string
[ -d "$MERT_TEST_EMB_ROOT" ] || { echo "FATAL: no real-audio MERT embeddings at $MERT_TEST_EMB_ROOT"; exit 1; }
echo "MERT emb root: $MERT_TEST_EMB_ROOT ($(ls $MERT_TEST_EMB_ROOT/*.npy 2>/dev/null | wc -l) files)"
ENTRY=/project/def-ichiro/pmohseni/music-alignment/extensions/hooks/run_eval_native_mert.py
