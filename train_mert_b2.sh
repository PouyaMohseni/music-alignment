...
#SBATCH --output=/project/def-ichiro/pmohseni/music-alignment/results/mert_b2-%j.log
#SBATCH --error=/project/def-ichiro/pmohseni/music-alignment/results/mert_b2-%j.log

# MERT audio-encoder swap (frozen, precomputed embeddings) COMBINED with
# B2's pitch-auxiliary loss and pitch-roll data (extensions/hooks/
# mert_pitch_patch.py -- a properly merged load_piece, since B1a's and B2's
# own load_piece patches would otherwise silently conflict). B2 alone
# (CBEncoder) scores 85.8% pct@0.5s; B1a alone (frozen MERT, no aux loss)
# scores 88.9% (current project best). Tests whether pitch-awareness adds
# anything on top of what's already the strongest base measured so far.
...
OUT=/project/def-ichiro/pmohseni/music-alignment/results/cb_ta_ext/MERT_B2_pitch_aux
...
echo "=== MERT+B2: frozen MERT audio encoder + pitch-auxiliary loss ==="
cd "$REPO/audio_conditioned_unet"

python /project/def-ichiro/pmohseni/music-alignment/extensions/hooks/run_train_mert_b2.py \
