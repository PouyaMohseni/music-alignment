#!/bin/bash
#SBATCH --job-name=eval-mert-b2
#SBATCH --account=def-ichiro
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=4:00:00
#SBATCH --output=/project/def-ichiro/pmohseni/music-alignment/results/eval_mert_b2-%j.log
#SBATCH --error=/project/def-ichiro/pmohseni/music-alignment/results/eval_mert_b2-%j.log

# Interim spot-check of MERT_B2_pitch_aux -- not a final result. Uses the
# MERT-aware native eval (extensions/hooks/run_eval_native_mert.py) since
# this checkpoint's audio_encoder is MERTProjector, not CBEncoder.
...
echo "=== Native MERT-aware eval_model.py: MERT_B2_pitch_aux on msmd_test ==="
