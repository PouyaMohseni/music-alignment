"""D2 training loop -- D1's model + decode, D2's MIDI-privileged losses (soft
repeat-aware CE + cross-modal distillation), both train-time only. Mirrors
D1's train.py structure (whole-piece-per-step, gradient accumulation,
ReduceLROnPlateau, checkpoint/resume) with a 3-term loss instead of 2.

    python -m mymodel.d2_midi_privileged.train --config configs/d2_midi_privileged.yaml [--resume]
"""
...
MAX_DISTILL_FRAMES = 512   # subsample long pieces for the O(T^2) InfoNCE term
...
def main():
    ...
    ap.add_argument('--config', default='configs/d2_midi_privileged.yaml')
