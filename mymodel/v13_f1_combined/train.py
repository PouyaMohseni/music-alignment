"""F1: E2/E3's v13-midi-privileged training (repeat-aware GT + MIDI
distillation) PLUS a third proven training-signal lever -- soft-DTW
position-trajectory consistency (the same mechanism C2 used successfully on
CPJKU's own architecture, extensions/losses/soft_dtw.py's soft_dtw_loss).
Stacks every training-signal improvement found this session into one model.
Sibling of mymodel/v13_midi_privileged/ (not an in-place edit -- that
directory's original train.py is used by a currently-running/queued SLURM
job; copying avoids any risk of touching what it reads mid-run).

New addition vs v13_midi_privileged/train.py: after computing the per-chunk
heatmap `pred` and repeat-aware GT `gt_batch` (dice loss target), ALSO decode
both to (x,y) positions via the same soft-argmax/center-of-mass utilities
B4/C2 used on CPJKU's architecture, normalize to [0,1] by (H, W_sc) (CRITICAL
-- B4's original bug was skipping this and getting a loss ~100x dice's
scale), and add dtw_weight * soft_dtw_loss(...) to the total loss. dtw_weight
starts small (0.1) so dice stays dominant for model selection (B3's lesson:
never let an aux loss hijack early stopping/best-checkpoint selection).

Usage:
    python -m mymodel.v13_f1_combined.train --config configs/v13_f1_combined.yaml
"""
...
def _forward_chunk(network, midi_encoder, perf_capture, score_1frame, feats, pitch_roll,
                   ...
                   distill_weight, dtw_weight, dtw_gamma, hidden, device):
    sl = len(frames)
    ...
    logs = {'dice': float(loss.detach()), 'distill': 0.0, 'dtw': 0.0}
    ...
    if dtw_weight > 0:
        # pred/gt_batch: (sl, 1, H, W_sc) -- soft_argmax_xy/center_of_mass_xy
        # operate on (..., H, W), giving (sl, 1, 2) = exactly soft_dtw_loss's
        # expected (T, B, 2) layout (T=sl BPTT timesteps, B=1), no reshape needed.
        pred_xy = soft_argmax_xy(pred.squeeze(1))         # (sl, 2) -- squeeze the channel dim first
        gt_xy = center_of_mass_xy(gt_batch.squeeze(1))     # (sl, 2)
        scale = pred_xy.new_tensor([max(W_sc - 1, 1), max(H - 1, 1)])
        pred_xy_n = (pred_xy / scale).unsqueeze(1)          # (sl, 1, 2) -- B=1
        gt_xy_n = (gt_xy / scale).unsqueeze(1)
        dtw = soft_dtw_loss(pred_xy_n, gt_xy_n, gamma=dtw_gamma)
        loss = loss + dtw_weight * dtw
...
def _train_epoch(network, midi_encoder, perf_capture, dataset, optimizer, seq_len, n_frames,
                 gt_width, alt_weight, distill_weight, dtw_weight, dtw_gamma, device):
    network.train(); midi_encoder.train()
    order = list(range(len(dataset)))
    np.random.shuffle(order)
    total_loss = total_acc = total_distill = total_dtw = n_seen = 0
    ...
    for idx in order:
        ...
        while t < T:
            ...
            loss, acc, hidden, logs = _forward_chunk(
                ...
                alt_weight, distill_weight, dtw_weight, dtw_gamma, hidden, device)
            loss.backward()
            optimizer.step()
            total_loss += logs['dice'] * sl
            total_distill += logs['distill'] * sl
            total_dtw += logs['dtw'] * sl
            total_acc += acc * sl
            n_seen += sl
            t += sl

    denom = max(1, n_seen)
    return total_loss / denom, total_acc / denom, total_distill / denom, total_dtw / denom


@torch.no_grad()
def _val_epoch(network, midi_encoder, perf_capture, dataset, seq_len, n_frames, gt_width,
              alt_weight, distill_weight, dtw_weight, dtw_gamma, device):
...
def main(cfg: DictConfig, resume: str | None = None):
    ...
    dtw_weight = cfg.loss.get('dtw_weight', 0.0)
    dtw_gamma = cfg.loss.get('dtw_gamma', 0.1)

    for epoch in range(start_epoch, max_epochs + 1):
        tr_loss, tr_acc, tr_distill, tr_dtw = _train_epoch(
            network, midi_encoder, perf_capture, train_ds, optimizer,
            seq_len=cfg.train.seq_len, n_frames=n_frames, gt_width=gt_width,
            alt_weight=alt_weight, distill_weight=distill_weight,
            dtw_weight=dtw_weight, dtw_gamma=dtw_gamma, device=device)
        val_loss = _val_epoch(
        ...
        print(f'epoch {epoch:3d}/{max_epochs}  '
              ...
              f'train_dtw={tr_dtw:.4f}  val_dice={val_loss:.4f}  '
