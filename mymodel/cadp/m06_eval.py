"""CADP M06 evaluation — continuous soft-argmax decode via the INR head.

Same sliding score-crop scan as M05 (eval can't center crops on ground truth
the way training does), but positions are continuous within each crop rather
than snapped to a x4 sub-column grid — this is what should show up as finer
localization (higher pct@0.05s/0.1s) relative to M05, if the INR head is
actually buying resolution beyond the tile-quantization ceiling.

    python -m mymodel.cadp.m06_eval \
        --checkpoint results/cadp_m06/best_model.pt \
            ...
            try:
                ...
                for t0 in range(0, T, win_frames):
                    ...
                    for c0 in crop_starts:
                        ...
                        px_lo = c0 * col_stride
                        px_hi = (c0 + n_cols_crop - 1) * col_stride + col_w
