"""CADP M01 evaluation — DTW on sim matrix, reports pct@0.5s.

    python -m mymodel.cadp.m01_eval \
        --checkpoint results/cadp_m01/best_model.pt \
    ...
    with open(out_root / 'per_piece.jsonl', 'w') as fout:
        for k, pid in enumerate(ds.piece_ids):
            try:
                ...
                # Convert column index → strip_x_px
                col_stride = float(piece['col_stride'])
                col_w      = float(ds.col_stride)   # pixel width (use stride as center ref)
