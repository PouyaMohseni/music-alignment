"""ConditionalUNet from CPJKU/audio_conditioned_unet (ismir-2020 branch).

Copied verbatim — only change is the import line for audio_encoder.
Key architectural details vs a naive reimplementation:
  - GroupNorm (not BatchNorm)
  - Additive skip connections (x = x + residual, not concat)
  - FiLM = gamma * x + beta (not (1+gamma)*x + beta)
  - n_filters_start=8 (very small), max_channel=128
...
from . import audio_encoders as audio_encoder
...
class ConditionalUNetBlock(nn.Module):
    def __init__(self, in_channels, out_channels, spec_out=128, film=True,
                 down_sample=True, up_sample=False, up_in_channels=1,
                 padding=1, no_skip=False):
    ...
    def forward(self, x, spec, residual=None):
        if self.up_sample:
            ...
            if residual is not None:
                ...
                if not self.no_skip:
                    x = x + residual   # ADDITIVE skip (not concat)
...
class ConditionalUNet(nn.Module):
    def __init__(self, config):
        ...
        self.bottleneck_block = ConditionalUNetBlock(
            ...
            self.rnn_size,
            film=self.n_encoder_layers + 1 in film_layers,
            down_sample=False)
