"""v10 training — CPJKU ConditionalUNet with MERT audio conditioning.

Same architecture/loss as v9, only the audio encoder changes:
CBEncoder (78-bin log-mel CNN) → MERTProjector (768-dim MERT → linear).

    python -m mymodel.v10_mert_unet.train --config configs/v10_mert_unet.yaml
"""
...
def _step(network, b, device):
    ...
    perf  = b['perf'].to(device).unsqueeze(0)         # (1, B, 1, 768, 1)
...
def main(cfg: DictConfig):
    ...
    # set_stats is no-op for MERTProjector — call for interface consistency
    network.perf_encoder.set_stats(*tl.dataset.compute_spec_stats())
