#!/bin/bash
#SBATCH --job-name=eval-c2
#SBATCH --account=def-ichiro
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=4:00:00
#SBATCH --output=/project/def-ichiro/pmohseni/music-alignment/results/eval_c2-%j.log
#SBATCH --error=/project/def-ichiro/pmohseni/music-alignment/results/eval_c2-%j.log

# Interim spot-check of C2_soft_dtw on its current best_model.pt -- training
# is still in progress as of submission, so this is not a final result.
#
...
# native portrait per-page format C2 was actually trained on (e.g. 1181x835,
...
CKPT_DIR=$(find results/cb_ta_ext/C2_soft_dtw/params -maxdepth 1 -name "*_C2_soft_dtw" -type d | sort | tail -1)
...
echo "=== Native eval_model.py: C2_soft_dtw on msmd_test ==="
