"""Decisive test: is a given frozen foundation-model score tower (+ trainable
projection/context adapter) usable in M1? Same overfit-one-piece bar Phase 1
validated for the from-scratch CNN tower (frame_acc 0.997). Generalizes
scripts/overfit_one_piece_m1_musvit.py to take --score_tower {musvit,dinov2}
instead of duplicating the harness per backbone.

Any non-CNN tower produces a DIFFERENT column count/spacing than D1's CNN
(w_downsample=4) convention, so onset positions are remapped into that tower's
own column space (proportionally, by physical strip-x fraction) rather than
reusing D1Piece's gt_cols directly.

    python -m scripts.overfit_one_piece_m1_tower --score_tower dinov2 \
        --config configs/d1_align_matrix.yaml [--steps 150] [--limit 12]
"""
...
def build_onset_columns_remapped(piece, w_col_tower, t_max=None, min_onsets=15):
    """Same GT-path construction as mymodel/m1_monotonic/data.py's
    build_onset_columns, but onset columns are computed in the TOWER's OWN
    column space (proportional strip-x fraction), not D1's w_downsample=4
    CNN columns."""
    ...
    onset_cols = np.clip(np.round(x_scaled_px / W_scaled * w_col_tower).astype(np.int64),
                         0, w_col_tower - 1)
    onset_cols = np.maximum.accumulate(onset_cols)   # enforce monotone (guards rounding ties)
    if onset_cols[0] != 0:
...
def main():
    ...
    ap.add_argument('--score_tower', required=True, choices=['musvit', 'dinov2'])
    ...
    ap.add_argument('--ce_sigma_cols', type=float, default=2.0)   # tower cols are coarser than CNN -> wider sigma
    ...
    print(f'Testing {a.score_tower} score tower on piece "{piece.piece_name}"  device={device}', flush=True)

    model = M1Model(**{**cfg['model'], 'score_tower': a.score_tower}).to(device)
    n_train = sum(p.numel() for p in model.parameters() if p.requires_grad)
    n_total = sum(p.numel() for p in model.parameters())
    print(f'params: {n_total:,} total, {n_train:,} trainable ({a.score_tower} frozen)', flush=True)

    # Probe the tower's own column count once (no grad) to build the onset mapping.
    with torch.no_grad():
        B_probe = model.score_tower(piece.strip.to(device))
    w_col_tower = B_probe.shape[0]
    print(f'{a.score_tower} score tower produces W_col={w_col_tower} columns for this piece', flush=True)

    built = build_onset_columns_remapped(piece, w_col_tower, t_max=a.t_max)
    assert built is not None, 'piece has too few onsets after truncation'
    T, onset_frames, onset_cols, gt_path = built
    N = len(onset_frames)
    print(f'T={T} frames  N={N} onset columns ({a.score_tower} space, span {onset_cols.min()}..{onset_cols.max()} '
          f'of {w_col_tower})', flush=True)
    ...
    fa, mae = decode_accuracy()
    print(f'\n=== {a.score_tower} score-tower overfit result ===', flush=True)
    ...
    if fa > 0.90:
        print(f'  {a.score_tower.upper()} TOWER PASSED overfit bar (matches CNN-tower Phase 1 standard).', flush=True)
    else:
        print(f'  {a.score_tower.upper()} TOWER DID NOT reach the CNN-tower overfit bar (>0.90) with this budget.',
              flush=True)
