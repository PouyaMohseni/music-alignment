#!/bin/bash
#SBATCH --job-name=eval-mert-b3
#SBATCH --account=def-ichiro
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=4:00:00
#SBATCH --output=/project/def-ichiro/pmohseni/music-alignment/results/eval_mert_b3-%j.log
#SBATCH --error=/project/def-ichiro/pmohseni/music-alignment/results/eval_mert_b3-%j.log

# Interim spot-check of MERT_B3_inr_subpixel -- not a final result. Uses
# the MERT-aware native eval (extensions/hooks/run_eval_native_mert.py)
# since this checkpoint's audio_encoder is MERTProjector, not CBEncoder.
...
CKPT_DIR=$(find results/cb_ta_ext/MERT_B3_inr_subpixel/params -maxdepth 1 -name "*_MERT_B3_inr_subpixel" -type d 2>/dev/null | sort | tail -1)
...
echo "=== Native MERT-aware eval_model.py: MERT_B3_inr_subpixel on msmd_test ==="
