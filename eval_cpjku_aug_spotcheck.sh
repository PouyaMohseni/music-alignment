...
# Spot-check eval of the in-progress cpjku-aug-train checkpoint -- not a
# final result.
#
...
# native portrait per-page format this checkpoint was actually trained on
# (e.g. 1181x835, third_party/cpjku_unet/data/msmd/msmd_test/score/<pid>_page_N.npz)
# -- a visual-domain mismatch far bigger than the spectrogram approximation
...
# the paper's ~85% number for the frozen official checkpoint. Eval is
# against the plain (non-augmented) msmd_test split -- test-time
# augmentation doesn't apply, matching every other experiment's eval.
...
echo "=== Native eval_model.py: cpjku_aug (CB_TA) on msmd_test ==="
