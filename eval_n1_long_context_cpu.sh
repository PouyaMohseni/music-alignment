#!/bin/bash
#SBATCH --job-name=eval-n1-longctx-cpu
#SBATCH --account=def-ichiro
#SBATCH --cpus-per-task=8
#SBATCH --mem=32G
#SBATCH --time=8:00:00
#SBATCH --output=/project/def-ichiro/pmohseni/music-alignment/results/eval_n1_long_context_cpu-%j.log
#SBATCH --error=/project/def-ichiro/pmohseni/music-alignment/results/eval_n1_long_context_cpu-%j.log

# Eval of N1 (two-tier memory Transformer temporal core).
...
CKPT_DIR=$(find results/cb_ta_ext/N1_long_context/params -maxdepth 1 -name "*_N1_long_context" -type d | sort | tail -1)
...
echo "=== Native eval_model.py: N1_long_context on msmd_test (CPU) ==="
python /project/def-ichiro/pmohseni/music-alignment/extensions/hooks/run_eval_native_n1_long_context.py \
    --param_path  "$CKPT" \
